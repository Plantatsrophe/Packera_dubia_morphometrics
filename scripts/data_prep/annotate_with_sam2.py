...
Description: Interactive Botanical Instance Segmentation Annotator powered by
             Segment Anything Model 2 (SAM 2) with multi-modal boundary controls
             (bounding boxes, exclusion points, freehand lasso, knife cutting,
             zoom/pan navigation, voucher advancement, reloading, and back navigation)
             for Packera specimens, outputting binary pixel masks tagged explicitly with
             botanical labels ('basal_leaf_whole', 'basal_leaf_partial', 'cauline_leaf').

Controls & Shortcuts (aligned with docs/SAM2_Precision_Botanical_Annotation_Guide.txt):
  - Left-Click: Add Point Prompt (Green foreground point in INCLUDE mode, Red exclusion point in EXCLUDE mode)
  - Right-Click: Add Negative Exclusion Point (Red dot, label 0)
  ...
  - Shift + Left-Drag: Draw Bounding Box Prompt (Constrains SAM 2 search space around rosette/leaf)
  - Ctrl + Left-Drag: Draw Knife / Cut Line across mask to sever roots/caudex from petioles
  - Keys 0-6: Assign Candidate Mask to Class
      0: basal_leaf_whole
      1: basal_leaf_partial
      2: cauline_leaf
      3: cauline_stem
      4: root_rhizome
      5: basal_rosette_clump
      6: capitulum
  - 'u': Undo last saved instance
  - 'c': Clear active candidate prompts, boxes, cut lines, and lasso paths
  - Enter or 'v': Save voucher annotations (binary PNG masks + normalized YOLO .txt) and advance
  ...
  - 'z' / 'x' (or Mousewheel): Zoom In / Zoom Out centered on mouse/viewport (1.0x to 6.0x)
...
# Fallback path inclusion for SAM 2 if not installed in current python site-packages
...
try:
    ...
    sys.stderr.write(f"Error importing SAM 2 module: {err}\n")
    sys.stderr.write("Ensure segment-anything-2 is installed or present at scripts/_archive/root_artifacts/segment-anything-2\n")
    sys.exit(1)

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
logger = logging.getLogger("SAM2Annotator")

# Botanical Schema explicitly featuring requested labels
...
# High-contrast BGR color palette for visual overlays
CLASS_COLORS: Dict[int, Tuple[int, int, int]] = {
    0: (0, 220, 0),       # basal_leaf_whole: Vibrant Green
    1: (100, 255, 100),   # basal_leaf_partial: Mint Green
    2: (0, 200, 255),     # cauline_leaf: Yellow-Green / Bright Cyan
...
class PrecisionSAM2Annotator:
    ...
    def __init__(
        ...
        logger.info(f"Loading SAM 2 predictor on device: {device} from {ckpt_p}...")
    ...
    def get_orig_coords(self, display_x: int, display_y: int, window_w: int, window_h: int) -> Tuple[int, int]:
        """Maps window display coordinates back to original native image coordinates."""
    ...
    def mouse_callback(self, event, x, y, flags, param):
        ...
        and multi-point prompt placement targeting dense basal rosettes.
        """
        ...
        if (flags & cv2.EVENT_FLAG_SHIFTKEY) and event == cv2.EVENT_LBUTTONDOWN:
            ...
            if (x2 - x1) > 10 and (y2 - y1) > 10:
                ...
                logger.info(f"Rosette Bounding Box prompt set: [{x1}, {y1}, {x2}, {y2}]")
        ...
        # 6. Standard Left-Click Point (Include vs Exclude depending on active mode)
    ...
    def update_candidate_mask(self):
        """Runs SAM 2 predictor inference on the active point and bounding box prompts."""
        ...
        try:
            ...
            # Re-apply any active cut lines across the candidate mask
    ...
    def apply_knife_cut(self, pt1: Tuple[int, int], pt2: Tuple[int, int]):
        """
        Carves a zero-mask cut line across the candidate mask to sever petiole/caudex overlap
        and retains the connected component containing active positive prompt points.
        """
        ...
        logger.info(f"Knife cut line applied between {pt1} and {pt2}")
    ...
    def load_existing_annotations(self, cat_num: str):
        """Loads existing YOLO polygon annotations and binary masks if previously saved."""
            ...
            with open(label_file, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if len(parts) >= 7:
                        ...
                        # Reconstruct boolean mask from polygon
                        mask = np.zeros((self.orig_h, self.orig_w), dtype=np.uint8)
    ...
    def save_current_sheet(self):
        """
        Saves all annotated instances for the current voucher sheet:
        1. Exports binary pixel masks tagged explicitly with label names ('basal_leaf_whole',
           'basal_leaf_partial', 'cauline_leaf', etc.) to PNG files in data/raw_annotations/masks/.
        2. Writes normalized YOLO polygon annotation file to data/raw_annotations/{catalogNumber}.txt.
        """
        cat_num = self.image_files[self.current_idx].stem
        label_file = self.output_dir / f"{cat_num}.txt"
        
        # 1. Write normalized YOLO polygon .txt
        ...
        # 2. Output binary pixel masks tagged explicitly with label names
        for idx, inst in enumerate(self.saved_instances):
            label_name = inst["label"]
            mask_bool = inst["binary_mask"]
            mask_png = (mask_bool.astype(np.uint8)) * 255
            
            mask_filename = self.masks_dir / f"{cat_num}_inst{idx:02d}_{label_name}.png"
            cv2.imwrite(str(mask_filename), mask_png)

        logger.info(f"Saved {len(self.saved_instances)} binary masks and YOLO annotations for {cat_num}")

    def render_display(self, window_w: int = 1280, window_h: int = 960) -> np.ndarray:
        """Renders interactive viewport, saved overlays, candidate mask, prompts, and HUD banner."""
        ...
        # 1. Render Saved Polygons with class color overlays
        for inst in self.saved_instances:
        ...
        # 2. Render Active Candidate Mask (Bright Cyan Boundary + Semi-transparent Fill)
        ...
        help_line2 = "0:basal_whole 1:basal_part 2:cauline_leaf 3-6:ext | 'u':Undo | 'c':Clear | Enter/'v':Save | WASD:Pan | 'z'/'x':Zoom"
        cv2.putText(display, help_line2, (20, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (0, 230, 255), 1, cv2.LINE_AA)

        return display

    def run(self):
        """Main annotation event loop with HighGUI controls."""
        window_name = "SAM 2 Precision Botanical Annotator (Packera)"
            ...
            while True:
                    ...
                    if self.current_candidate_mask is not None:
                        ...
                        if poly:
                            ...
                            self.saved_instances.append({
                                ...
                                "label": label_name,
                                "polygon": poly,
                                "binary_mask": self.current_candidate_mask.copy()
                            })
                            logger.info(f"Assigned instance -> Class {class_id} [{label_name}] ({len(poly)//2} vertices)")
                        else:
                ...
                # 'b' Key -> GO BACK TO PREVIOUS VOUCHER
                    ...
                    if self.current_idx > 0:
                        ...
                        logger.info("Already at first voucher in queue.")

                # 'n' Key -> SKIP FORWARD TO NEXT VOUCHER
                elif key in (ord('n'), ord('N')):
                    self.current_idx += 1
                    logger.info("Skipping forward to next voucher...")
                    break

                # Enter (13, 10) or 'v' Key -> SAVE & ADVANCE
                elif key in (13, 10, ord('v'), ord('V')):
                ...
                # Undo Last Saved Instance ('u')
                    ...
                    if self.saved_instances:
                        ...
                        logger.info(f"Undid instance: Class {removed['class_id']} [{removed['label']}]")
...
def parse_args():
    parser = argparse.ArgumentParser(description="SAM 2 Precision Botanical Annotator for Packera")
    parser.add_argument("--images-dir", type=str, default="data/raw_vouchers", help="Directory containing raw voucher images")
    parser.add_argument("--output-dir", type=str, default="data/raw_annotations", help="Directory to save annotations and binary pixel masks")
    ...
    parser.add_argument("--start-from-beginning", action="store_true", help="Start from first voucher in queue even if already annotated")
