...
class GeometricMetrics:
    ...
    Attributes:
        mask_area: Total foreground pixel count (Area_mask).
        min_area_rect_area: Area of the minimum bounding oriented rectangle.
        rectangularity: Ratio Area_mask / Area_minAreaRect (0.0 to 1.0).
        convex_hull_area: Area of the contour convex hull.
        solidity: Ratio Area_mask / Area_convex_hull (0.0 to 1.0).
        num_approx_vertices: Number of vertices after Douglas-Peucker polygon approximation.
        corner_angles_deg: List of computed internal corner angles in degrees.
        is_rectangular: Flag set True if rectangularity exceeds threshold (> 0.86).
        is_orthogonal_quad: Flag set True if polygon has 4 vertices with all angles in [80°, 100°].
        is_valid_solidity: Flag set True if solidity satisfies minimum intact leaf requirement (>= 0.72).
    """
    mask_area: float
...
class SpectralMetrics:
    ...
    Attributes:
        mean_hue: Mean Hue channel value in degrees [0, 360].
        mean_saturation: Mean Saturation channel value normalized to [0.0, 1.0].
        mean_value: Mean Value/Brightness channel value normalized to [0.0, 1.0].
        high_saturation_pixel_count: Number of foreground pixels with Saturation > 0.45.
        high_saturation_ratio: Fraction of foreground mask with Saturation > 0.45.
        is_color_swatch: Flag set True if high_saturation_ratio exceeds threshold (> 0.15).
    """
    mean_hue: float
...
class TextureMetrics:
    ...
    Attributes:
        laplacian_variance: Variance of the Laplacian second derivative across interior pixels.
        canny_edge_density: Ratio of Canny edge pixels to total interior foreground pixels.
        horizontal_stroke_density: Relative magnitude of horizontal Sobel gradients (text lines).
        vertical_stroke_density: Relative magnitude of vertical Sobel gradients (character glyphs).
        is_printed_text: Flag set True if texture profile indicates printed typographic text.
    """
...
class FilterResult:
    ...
    Attributes:
        is_valid: Boolean indicating whether candidate is an authentic botanical leaf.
        status: Categorical status string ('VALID_LEAF', 'REJECTED_ARTIFACT', 'ROUTED_ANNOTATION').
        primary_rejection_reason: Descriptive reason string if rejected, otherwise None.
        reclassified_category: New assigned category if reclassified (e.g. 'color_chart', 'annotation_label').
        geometric_metrics: Detailed GeometricMetrics instance.
        spectral_metrics: Detailed SpectralMetrics instance.
        texture_metrics: Detailed TextureMetrics instance.
        routed_file_path: Absolute path where artifact patch was archived, if applicable.
    """
    is_valid: bool
    status: str
