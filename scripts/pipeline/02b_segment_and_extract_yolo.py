...
# =============================================================================
# Direct Ultralytics YOLO Inference Engine
# =============================================================================

class UltralyticsInferenceEngine:
    """Encapsulates Ultralytics YOLO instance segmentation model loading and inference."""
    ...
    def _load_model(self) -> None:
        """Loads Ultralytics YOLO model from weights checkpoint."""
        try:
            from ultralytics import YOLO
        except ImportError as e:
            logger.warning(f"Ultralytics not available: {e}. Running in simulation/fallback mode.")
            return

        if not self.weights_path.exists():
            logger.warning(f"Weights file not found at {self.weights_path}, running with uninitialized weights/default for testing.")

        # Map 'cuda' string to device '0' for ultralytics if needed
        yolo_device = 0 if self.device == "cuda" else "cpu"

        # Load the model
        try:
            self.model = YOLO(str(self.weights_path.resolve()))
            logger.info(f"Loaded YOLO model from {self.weights_path} on {yolo_device}")
    ...
    def predict(self, image_bgr: np.ndarray) -> List[Tuple[Tuple[int, int, int, int], np.ndarray, float, int]]:
        ...
        # YOLO runs natively on BGR images
        # imgsz=1024 or higher could be passed, but the model remembers its trained size usually.
        results = self.model.predict(
            source=image_bgr,
            conf=self.score_thresh,
            device=0 if self.device == "cuda" else "cpu",
            verbose=False,
            retina_masks=True  # Helpful for higher resolution masks
        )
        ...
        if result.masks is not None and result.boxes is not None:
            ...
            # Use polygons scaled back to original image size
            ...
            for i in range(len(boxes)):
                ...
                mask_uint8 = np.zeros(image_bgr.shape[:2], dtype=np.uint8)
                poly = np.array(polygons[i], dtype=np.int32)
                if len(poly) > 0:
                    cv2.fillPoly(mask_uint8, [poly], 255)
                
                out_results.append(((ymin, xmin, ymax, xmax), mask_uint8, float(scores[i]), int(classes[i])))
...
class SegmentAndExtractPipeline:
    ...
    def __init__(
        ...
        self.model_engine = UltralyticsInferenceEngine(
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Direct Ultralytics YOLO segmentation, 2-path leaf extraction, and contour coordinate export."
    )
    parser.add_argument(
        "--vouchers",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Path to fine-tuned YOLO model checkpoint."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
